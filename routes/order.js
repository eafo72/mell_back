...
app.post("/checkStock", async (req, res) => {
  ...
  try {
    ...
    for (let i = 0; i < descripcion.length; i++) {
      ...
      const ifAvailableOne = await Stock.find({
        codigo: descripcion[i]["codigo"],
        stock: { $gte: descripcion[i]["cantidad"] },
      });
      ...
      if (ifAvailableOne.length == 0) {
        res.json({
          ...
          msg:
            ...
            " no tiene el stock necesario para este pedido",
        });
        return;
      }
      ...
      const calculo =
        ifAvailableOne[0].stock -
        ifAvailableOne[0].apartado -
        descripcion[i]["cantidad"];
...
app.put("/actualizar", async (req, res) => {
  ...
  try {
    const updatePedido = await Pedidos.findByIdAndUpdate(id, {}, { new: true });
...
app.put("/abonar", async (req, res) => {
  ...
  try {
    ...
    const updatePedido = await Pedidos.findByIdAndUpdate(
      id,
      {
...
app.post("/prepedido_actualizar", async (req, res) => {
  const { id_pedido, indice, status } = req.body;

  if (id_pedido == undefined) {
    res.status(500).json({
      msg: "No se recibió el id_pedido",
    });
  } else if (indice == undefined) {
    res.status(500).json({
      msg: "No se recibió el indice",
    });
  } else if (status == undefined) {
    res.status(500).json({
      msg: "No se recibió el status",
    });

  } else {
    try {
      const updatePrePedido = await PrePedidos.findByIdAndUpdate(
        id_pedido,
        {
			$set: { [`descripcion.${indice}.status`]: status},
        },
        { new: true }

      );

      res.json({ updatePrePedido });
    } catch (error) {
      res.status(500).json({
        msg: "Hubo un error actualizando el PrePedido " + error,
      });
    }
  }
});
